...
test('should book an object', async () => {
    ...
    const retrievedObjStatus2 = await client.events.retrieveObjectStatus(event.key, 'A-2')
...
test('should book an object with sections', async () => {
    ...
    const retrievedObjStatus1 = await client.events.retrieveObjectStatus(event.key, 'Section A-A-1')
    const retrievedObjStatus2 = await client.events.retrieveObjectStatus(event.key, 'Section A-A-2')
    expect(retrievedObjStatus1.status).toEqual(objectStatus.BOOKED)
    expect(retrievedObjStatus2.status).toEqual(objectStatus.BOOKED)
