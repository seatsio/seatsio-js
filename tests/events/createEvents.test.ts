...
test('it supports channels', async () => {
    ...
    expect(events[0].channels).toEqual([
        new Channel({ id: events[0].channels![0].id, key: 'channelKey1', name: 'channel 1', color: 'blue', index: 1, objects: ['A-1', 'A-2'], areaPlaces: { GA1: 5 } }),
        new Channel({ id: events[0].channels![1].id, key: 'channelKey2', name: 'channel 2', color: 'red', index: 2, objects: ['A-3'], areaPlaces: { GA1: 3 } })
    ])
})
