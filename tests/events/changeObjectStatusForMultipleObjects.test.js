...
test('should change object status for multiple objects as an array of classes with general admission', async () => {
    ...
    await testUtils.createTestChart(chartKey, user.designerKey);
    var event = await client.events.create(chartKey);
    var objects = [{objectId: 'A-1'}, {objectId: 'GA1', quantity: 5}];

    await client.events.changeObjectStatus(event.key, objects, 'lolzor');
    var objStatusA1 = await client.events.retrieveObjectStatus(event.key, 'A-1');
    var objStatusGA1 = await client.events.retrieveObjectStatus(event.key, 'GA1');
...
test('should change object status for multiple objects as an array of classes and strings', async () => {
    ...
    await testUtils.createTestChart(chartKey, user.designerKey);
    var event = await client.events.create(chartKey);
    var objects = ['A-1', {objectId: 'GA1', quantity: 5}];
...
test('should change object status for multiple objects with quantity', async () => {
    ...
    await client.events.changeObjectStatus(event.key, objects, 'lolzor');
    var status1 = await client.events.retrieveObjectStatus(event.key, 'GA1');
    var status2 = await client.events.retrieveObjectStatus(event.key, '34');
...
test('should change object status for multiple objects, GAs and Seats', async () => {
    ...
    await client.events.changeObjectStatus(event.key, objects, 'lolzor');
    var status1 = await client.events.retrieveObjectStatus(event.key, 'GA1');
    var status2 = await client.events.retrieveObjectStatus(event.key, 'A-1');
