...
test('should list first page of active subaccounts with page size', async () => {
    ...
    expect(firstPage.items[0].id).toEqual(subaccounts[2].id)
...
test('should list page after of active subaccounts', async () => {
    ...
    let pageAfter = await client.subaccounts.active.pageAfter(subaccounts[2].id)
...
test('should list page after of active subaccounts with page size', async () => {
    ...
    let pageAfter = await client.subaccounts.active.pageAfter(subaccounts[2].id, null, 1)

    expect(pageAfter.items[0].id).toEqual(subaccounts[1].id)
...
test('should list page before of active subaccounts', async () => {
    ...
    let pageAfter = await client.subaccounts.active.pageBefore(subaccounts[0].id)

    expect([pageAfter.items[0].id, pageAfter.items[1].id].sort()).toEqual([subaccounts[1].id, subaccounts[2].id].sort())
...
test('should list page before of active subaccounts with page size', async () => {
    ...
    let pageAfter = await client.subaccounts.active.pageBefore(subaccounts[0].id, null, 1)
