...
describe('list active subaccounts', () => {
    beforeAll(async () => {
        jest.setTimeout(35000)
        global.testUser = await testUtils.createTestUser()
        global.testClient = testUtils.createClient(testUser.secretKey)

        global.subaccount1 = await testClient.subaccounts.create()
        global.subaccount2 = await testClient.subaccounts.create()
        global.subaccount3 = await testClient.subaccounts.create()
    })
    ...
    test('should list first page of active subaccounts', async () => {
        let firstPage = await testClient.subaccounts.active.firstPage()

        expect(firstPage.items.map(item => item.id).sort()).toEqual([subaccount1.id, subaccount2.id, subaccount3.id, testUser.mainWorkspace.primaryUser.id].sort())
        expect(firstPage.items.length).toBe(4)
    })

    test('should list first page of active subaccounts with page size', async () => {
        let firstPage = await testClient.subaccounts.active.firstPage(null, 1)

        expect(firstPage.items[0].id).toEqual(subaccount3.id)
        expect(firstPage.items.length).toBe(1)
    })

    test('should list page after of active subaccounts', async () => {
        let pageAfter = await testClient.subaccounts.active.pageAfter(subaccount3.id)

        expect([pageAfter.items[0].id, pageAfter.items[1].id, pageAfter.items[2].id].sort()).toEqual([subaccount1.id, subaccount2.id, testUser.mainWorkspace.primaryUser.id].sort())
        expect(pageAfter.items.length).toBe(3)
    })

    test('should list page after of active subaccounts with page size', async () => {
        let pageAfter = await testClient.subaccounts.active.pageAfter(subaccount3.id, null, 1)
    ...
    test('should list page before of active subaccounts', async () => {
        let pageAfter = await testClient.subaccounts.active.pageBefore(subaccount1.id)
    ...
    test('should list page before of active subaccounts with page size', async () => {
        let pageAfter = await testClient.subaccounts.active.pageBefore(subaccount1.id, null, 1)
